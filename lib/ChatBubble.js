  ...
  _createClass(ChatBubble, [{
    ...
    value: function getConversations(messages) {
      ...
      var listItems = messages.map(function (message, index) {
        ...
        var message_is_link = false;
        
        if (message.text.indexOf('<title>') != -1 &&
          message.text.indexOf('</title>') != -1 &&
          message.text.indexOf('<link>') != -1 &&
          message.text.indexOf('</link>')) {
            message_is_link = true;
            var file_name = message.text.substring(
              message.text.indexOf('<title>') + 7,
              message.text.lastIndexOf('</title>'));
        
            var file_link = message.text.substring(
              message.text.indexOf('<link>') + 6,
              message.text.lastIndexOf('</link>'));
                
            var text = '<a href="' + file_link  + '" target="_blank">' +
              file_name + '</a>'
              message.text = text;
        }
